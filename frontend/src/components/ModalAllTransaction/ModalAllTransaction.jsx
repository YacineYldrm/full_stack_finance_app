...
const ModalAllTransaction = ({transactions,modal,setModal}) => {
    ...
    useEffect(()=>{
        const filterResult = transactions?.filter((transaction)=>{
            if(search !== "" && date!==""){
                return JSON.stringify(transaction).toLowerCase().includes(search.toLowerCase()) && new Date(transaction.date - (new Date().getTimezoneOffset()*60000)).toISOString().slice(0,10)===date
            } else if(search!=="" && date===""){
                return JSON.stringify(transaction).toLowerCase().includes(search.toLowerCase())
            } else if(search=="" && date!==""){
    ...
    return ( 
        ...
        <section>
            ...
            <div>
                <p>{search}</p>
                <div>
                    {result?.map((transaction)=><Transaction transaction={transaction}/>)}
                </div>
            </div>
        </section>
